...
def run_internal_models(df_ext_valid, device):
    ...
    return {
        ...
        "Stream 2 (Ranking)": rank_p_rnk_ext,
    ...
    for auc, name, pred in items:
        ...
        if name == "Our Proposed Model":
            ax.plot(
                ...
                lw=3.6,
                alpha=1.0,
                label=f"Our Proposed Model (AUC={auc:.3f})",
                zorder=12,
            )
...
def main():
    parser = argparse.ArgumentParser(description="External multi-model validation (inference-only).")
    parser.add_argument("--external-tag", type=str, default=None)
    parser.add_argument(
        "--external-file",
        type=str,
        default=None,
        help="Optional absolute/relative path to external Excel. If provided, it overrides tag-based default path.",
    )
    ...
    if args.external_file is not None:
        ...
        if not os.path.isabs(ext_raw_path):
        ...
        inferred_tag = os.path.splitext(os.path.basename(ext_raw_path))[0]
        external_tag = args.external_tag if args.external_tag is not None else inferred_tag
    else:
        external_tag = args.external_tag if args.external_tag is not None else EXTERNAL_TAG
        ext_raw_path = os.path.join(base_dir, f"results_flat_{external_tag}", f"external_cleaned_{external_tag}.xlsx")
    ...
    print(f"[INFO] external tag: {external_tag}")
    ...
    threshold_path = os.path.join(output_dir, f"external_threshold_sensitivity_specificity_curve_{external_tag}.csv")
    ...
    comp_path = os.path.join(output_dir, f"external_comparison_table_{external_tag}.csv")
    df_comp.to_csv(comp_path, index=False, encoding="utf-8-sig")

    pred_out = pd.DataFrame({"target": y_true})
    for k, v in all_preds.items():
        pred_out[k] = v
    pred_path = os.path.join(output_dir, f"external_model_predictions_{external_tag}.csv")
    pred_out.to_csv(pred_path, index=False, encoding="utf-8-sig")

    roc_path = os.path.join(output_dir, f"external_roc_comparison_{external_tag}.png")
