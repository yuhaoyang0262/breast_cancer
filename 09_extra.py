...
if __name__ == "__main__":
    ...
    threshold_table_path = os.path.join(config.OUTPUT_DIR, "external_threshold_sens_spec_with_ci.csv")
    threshold_perf_df.to_csv(threshold_table_path, index=False, encoding='utf-8-sig')

    auc_ci_path = os.path.join(config.OUTPUT_DIR, "external_auc_with_ci.csv")
    pd.DataFrame([
        {
            'model': best_name,
            'auc': auc_champion,
    ...
    # --- 最终保存 ---
    output_path = os.path.join(config.OUTPUT_DIR, "Figure4_External_Validation.pdf")
